...
public class PlatformController : MonoBehaviour
{
    ...
    public enum PlatformModes { Mode_Servo, Mode_8Bit, Mode_Float32 };
    ...
    public bool Init(string _com, int _baud)
    {
        ...
        // Alternate Serial Thread option
        //SerialThreadIO.instance.Init(_com, _baud);
        //serialPort = SerialThreadIO.instance.serialPort;
    ...
    public void SendSerial()
    {
        ...
        if (mode == PlatformModes.Mode_Servo)
        {
        ...
        {
            serialPort.Write("F"); // message type: F indicates Float32 mode
            byte[] byteArray = new byte[24]; // start, 6 * 4 bytes, end == 26
            ...
            //SerialThreadIO.instance.EnqueueBytes(byteArray);
        }

        serialPort.Write(endFrame); // start frame of message
    }

    public void SendASCII(string cmd)
    {
        serialPort.Write(startFrame);
        serialPort.Write("A" + cmd + "\n"); // A for ASCII, command string, new line terminator
    }
    ...
    public void HomePlatform()
    {
        if (useSliders)
        {
            ...
            if (mode == PlatformModes.Mode_8Bit || mode == PlatformModes.Mode_Servo)
            {
                for (int i = 0; i < byteValues.Length; i++)
                {
                    byteValues[i] = 128;
    ...
    public void UpdateValuesFromSliders()
    {
        for (int i = 0; i < sliders.Length; i++)
        {
            ...
            else if (mode == PlatformModes.Mode_8Bit || mode == PlatformModes.Mode_Servo) { byteValues[i] = (byte)sliders[i].value; }
        }
    }
